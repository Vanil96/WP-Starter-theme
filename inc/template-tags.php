...
if ( ! function_exists( 'wps_entry_footer' ) ) {
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function wps_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'wps' ) );
			if ( $categories_list && wps_categorized_blog() ) {
...
if ( ! function_exists( 'wps_categorized_blog' ) ) {
	/**
	 * Returns true if a blog has more than 1 category.
	 *
	 * @return bool
	 */
	function wps_categorized_blog() {
		...
		if ( $all_the_cool_cats > 1 ) {
			// This blog has more than 1 category so wps_categorized_blog should return true.
			return true;
		}
		// This blog has only 1 category so wps_categorized_blog should return false.
		return false;
	}
}

add_action( 'edit_category', 'wps_category_transient_flusher' );
add_action( 'save_post', 'wps_category_transient_flusher' );

if ( ! function_exists( 'wps_category_transient_flusher' ) ) {
	/**
	 * Flush out the transients used in wps_categorized_blog.
	 */
	function wps_category_transient_flusher() {
...
if ( ! function_exists( 'wps_body_attributes' ) ) {
	/**
	 * Displays the attributes for the body element.
	 */
	function wps_body_attributes() {
		/**
		 * Filters the body attributes.
		 *
		 * @param array $atts An associative array of attributes.
		 */
		$atts = array_unique( apply_filters( 'wps_body_attributes', $atts = array() ) );
...
if ( ! function_exists( 'wps_edit_post_link' ) ) {
	/**
	 * Displays the edit post link for post.
	 */
	function wps_edit_post_link() {
		edit_post_link(
			sprintf(
...
if ( ! function_exists( 'wps_link_pages' ) ) {
	/**
